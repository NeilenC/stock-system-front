  ...
  return (
    ...
    <Box sx={{ display: 'flex', alignItems: 'center' }}>
      ...
      {Array.from({ length: end - start + 1 }, (_, index) => (
        <Button
          key={start + index}
          variant={currentPage === start + index ? "contained" : "outlined"}
          size="small" // Tamaño pequeño
          ...
          sx={{
            ...
            backgroundColor: currentPage === start + index ? 'primary.dark' : 'primary.light',
            color: currentPage === start + index ? 'white' : 'primary.dark',
            '&:hover': {
              backgroundColor: currentPage === start + index ? 'primary.dark' : 'grey.300',
            },
          }}
        >
          {start + index}
    ...
    <Button
      ...
      onClick={() => onPageChange(currentPage + 1)}
      sx={{ ml: 1, maxWidth: '80px', height: currentPage === count ? '40px': '38px', 
        '&:hover': {
          color:'white'
        },
       }}
